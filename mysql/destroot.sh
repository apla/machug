...
install_dirs () {
	...
	mkdir -p "$INSTPREFIX$1/var/log/mysql"
	chown _mysql:_mysql "$INSTPREFIX$1/var/log/mysql"

}

rm -rf $INSTPREFIX/mysql

install_dirs '/mysql'
make $INSTALLDIRS install

mv "$INSTPREFIX/mysql$PREFIX/mysql-test" "$INSTPREFIX/mysql$PREFIX/share/mysql-test"
mv "$INSTPREFIX/mysql$PREFIX/sql-bench" "$INSTPREFIX/mysql$PREFIX/share/sql-bench"

chown root:wheel "$INSTPREFIX/mysql$PREFIX"

mkdir -p $INSTPREFIX/mysql/Library/LaunchDaemons
mkdir -p $INSTPREFIX/mysql/etc

cat > $INSTPREFIX/mysql/Library/LaunchDaemons/com.mysql.mysqld.plist <<InputComesFromHERE
...
cat > $INSTPREFIX/mysql/etc/my.cnf <<InputComesFromHERE
